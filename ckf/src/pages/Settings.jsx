...
function MarketingVoiceovers() {
  ...
  async function del(item) {
    ...
    try {
      // Route to the right table based on which kind the row came from.
      if (item.kind === 'creative') {
        await call('mktg-vo', { action: 'delete_creative', creative_id: item.creative_id });
      } else {
        await call('mktg-vo', { action: 'delete', draft_id: item.draft_id });
      }
      load();
    } catch (e) { setErr(e.message); }
  }
  ...
  return (
    <div className="card" style={{ marginBottom: 12 }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-dim)', marginBottom: 10 }}>
        MP3s generated by ElevenLabs from ad briefs and creative scripts. The
        link is public — your editor opens it in a browser to download. Generate
        from the{' '}
        <Link to="/business/marketing/creative">Creative pipeline</Link> (final step)
        or the legacy{' '}
        <Link to="/business/marketing/assistant">Assistant queue</Link>.
      </div>
      {!items ? <div className="loading">Loading…</div> :
        items.length === 0 ? (
          <div className="empty" style={{ fontSize: 12 }}>
            No voiceovers yet. Generate one from a brief or creative.
          </div>
        ) : (
          <>
            {items.map((v) => {
              const rowKey = v.kind === 'creative' ? `c:${v.creative_id}` : `d:${v.draft_id}`;
              const idForUi = v.kind === 'creative' ? v.creative_id : v.draft_id;
              return (
                <div key={rowKey} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, marginBottom: 2, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span>{v.label}</span>
                    <span className="pill" style={{ fontSize: 10 }}>{v.kind === 'creative' ? (v.creative_type || 'creative') : 'draft'}</span>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => copyUrl(v.public_url, idForUi)} className="primary" style={{ padding: '6px 12px', fontSize: 12 }}>
                      {copiedId === idForUi ? 'Copied' : 'Copy'}
                    </button>
                    ...
                    <button onClick={() => del(v)} className="danger" style={{ padding: '6px 10px', fontSize: 12 }}>Del</button>
                  </div>
                </div>
              );
            })}
