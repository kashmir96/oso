...
Don't create vague goals. For numeric, ask one question if target is missing.`,
  ...
  // pipeline state in one place.
  {
    ...
    description: "Drive the chat-conversational creative pipeline. ONLY use in business chat when Curtis says 'marketing mode' / 'let's make an ad' / 'create an ad' / similar.\n\nIMPORTANT (architecture): you DO NOT run the slow generation stages (strategy, variants, outline, hooks, draft, critique) yourself. Those are dispatched by the CLIENT directly. Your job is brief intake + final lifecycle (approve, voiceover, submit_to_assistant).\n\nBRIEF INTAKE — DO NOT INTERROGATE.\nCurtis writes informative messages. Read his trigger message thoroughly and EXTRACT what's there before asking anything. Only TWO things truly require Curtis to specify:\n  - objective (what is this creative for?)\n  - audience (who is this aimed at?)\nEverything else has a default:\n  - creative_type: infer from his words. 'video', 'reel', 'script', 'voiceover', 'shorts' -> video_script. 'static', 'image', 'carousel', 'photo' or no mention -> ad.\n  - platform: default 'meta' unless he names a different one.\n  - format: default 'video' for video_script, 'static' for ad. Override if he says reel/carousel/etc.\n  - length_or_duration: null unless mentioned.\n  - kpi_target: { metric: 'purchase', target_value: null } unless he names a different metric.\n  - constraints: extract any 'no X / must include Y' lines, else empty array.\n\nDECISION TREE (one turn ideally):\n  - If his trigger message has objective + audience: call intake_brief IMMEDIATELY with all the inferred defaults applied. Reply: \"Got it — generating strategy now.\" Do NOT ask follow-ups.\n  - If exactly 1 thing missing (just objective OR just audience): ask one focused question. Don't ask about format/KPI/platform — those have defaults.\n  - If both objective AND audience missing: ask ONE batched question covering both, naming any extras you've already inferred. e.g. \"What's it for and who's it aimed at? (I've got: video script, meta, no constraints — change anything?)\"\n  - NEVER ask 3+ separate questions. NEVER ask things he already said.\n\nAFTER intake_brief succeeds the client takes over — pipeline cards appear inline. DON'T comment on them. Let Curtis interact.\n\nFINAL LIFECYCLE (after critique card is approved):\n  - call action='approve' with a brief approval_reason from his words.\n  - ask 'Generate voiceover?' (script-bearing creatives only). On yes: action='generate_voiceover'. Give the public_url.\n  - ask 'Proceed to Assistant?' On yes: action='submit_to_assistant'. Reply with the detail_url.\n\nLATENCY GUARDRAIL: never call this tool more than ONCE per turn. Never combine with another tool call in the same response.",
    input_schema: {
      type: 'object',
      properties: {
        action: {
